...
def monitor_cpu_usage():
    ...
    if cpu_usage > 90:
        ...
        # If a specific process (e.g., high memory usage or runaway process) is causing issues
        if processes[0][2] > 90:
            ...
            # Take action: Restart the process or reboot the system
            try:
                print("Terminating the high CPU usage process...")
                psutil.Process(processes[0][0]).terminate()
                print(f"Terminated process {processes[0][1]}.")
                time.sleep(2)  # Wait for the process to be terminated

                # Optionally, you can restart the application if needed (e.g., for a web server)
                # os.system("command_to_restart_application")  # For example: os.system("python web_server.py")
            
            except Exception as e:
                print(f"Error terminating process: {e}")

            # Optionally reboot the system if CPU usage is consistently high
            # Uncomment the next line to enable reboot
            # os.system("sudo reboot")  # On Linux/Unix systems
